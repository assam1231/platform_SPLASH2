...
namespace ispa
{
    ...
    enum router_type {req, rsp};
		...
		{
			private:
				enum direction {N, W, S, E, PE, MEM, NUM_DIR, DIR_ERROR};
				...
				void thread(unsigned int my_dir)
				{
					...
					while(true)
					{
						...
						if(trans == 0)
						{
							...
							ROUTER_DEBUG(
							std::cout << std::setw(12) << sc_core::sc_time_stamp() << " thr " << name() 
                        << "(" << row_ << "," << col_ << ") " 
        ...
        void create_memory_port() 
        {
          if(type_==req)
          {
            initSockets[MEM] = new tlm_utils::simple_initiator_socket_tagged
              <TlmMeshRouter, data_width_g>;

						initSockets[MEM]->register_nb_transport_bw
							(this, &TlmMeshRouter::nb_transport_bw, MEM);
          }
          else if(type_==rsp)
          {
            targetSockets[MEM] = new tlm_utils::simple_target_socket_tagged
              <TlmMeshRouter, data_width_g>;

						targetSockets[MEM]->register_nb_transport_fw
							(this, &TlmMeshRouter::nb_transport_fw, MEM);
          }


          sc_core::sc_spawn(sc_bind(&TlmMeshRouter::thread, this, MEM));

            ROUTER_DEBUG(
              if (type_==req){
                std::cout << "Request router[" << row_ << "][" << col_ << "]" 
                          << "Spawn thread for dir " << dir_name[MEM] << std::endl;
              }
              else {
                std::cout << "Response router[" << row_ << "][" << col_ <<"]" 
                          << "Spawn thread for dir " << dir_name[MEM] << std::endl;
          ...
          << std::setw(20) << "Src: " << NocPayloadExt::get_source_id(trans) 
					...
					for(unsigned int i = 0; i < NUM_DIR; ++i)
					{
					  unsigned int source = (top_priority[dir]+i) % NUM_DIR;
						...
						{ 
							...
							<< std::setw(20) << "Src: " << NocPayloadExt::get_source_id(*trans_ptr_ptr)
							<< ", Dest: " << (*trans_ptr_ptr)->get_address() 
          ...
          return DIR_ERROR;
				...
				{
					unsigned long int rowPart;
					unsigned long int colPart;
          bool is_to_mem;
          if (rank_id >= 16) //external memory
          {
            is_to_mem = true;

					  rowPart = (rank_id-16) / n_cols_; 
					  colPart = (rank_id-16) - row_*n_cols_;
          }
          else 
          {
            is_to_mem = false;
...
const char TlmMeshRouter<data_width_g>::dir_name[] = {'N', 'W', 'S', 'E', 'P', 'M'};
}
